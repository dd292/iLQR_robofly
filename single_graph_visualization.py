...
class TrajectoryVisualize():
    ...
    def plotter(self, dict, box_qp = False ):
        ...
        for key,value in dict.items():
            if key=='angles':
                self.plot_angles(fig, [axs[0,0], axs[0,1]], value[:,0],value[:,1],value[:,2],value[:,3],value[:,4],value[:,5], comparison)
            elif key=='pos':
                self.plot_positions(fig, [axs[1,0], axs[1,1], axs[2,0]], value[:,0],value[:,1],value[:,2],value[:,3],value[:,4],value[:,5], comparison)
            elif key == 'vel':
                self.plot_vel(fig, [axs[2,1], axs[2,1], axs[3,0]], value[:,0],value[:,1],value[:,2],value[:,3],value[:,4],value[:,5], comparison)
            elif key == 'omega':
                #self.plot_omegas(fig, axs[8:10], value[:,0],value[:,1],value[:,2],value[:,3],value[:,4],value[:,5], comparison)
                continue
            elif key == 'input':
                self.plot_input(fig, box_qp,  [axs[3,1], axs[4,0], axs[4,1]], value[:, 0], value[:, 1], value[:, 2], comparison)
            self.figure_number+=1

    def show_plot(self):

        plt.savefig('results/01_gamma_noise.png')
        plt.show()
